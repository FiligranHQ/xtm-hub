...
test('should confirm CRUD of users is ok', async ({ page }) => {
    ...
    await page.getByPlaceholder('Email', { exact: true }).fill('userInE2E@thales.com');
    ...
    await page.getByLabel('Thales').press('Enter');
    await page.getByRole('button', { name: 'Validate' }).click();
    await expect(page.getByRole('cell', { name: 'userInE2E@thales.com' })).toBeVisible();
    ...
    await page.getByRole('button', { name: 'Validate' }).click();
    await expect(page.getByRole('heading', { name: 'firstnameModified lastname' })).toBeVisible();
    ...
    await expect(page.getByRole('cell', { name: 'userInE2E@thales.com' })).not.toBeVisible();
});
