...
test('should confirm CRUD of documents is OK', async ({ page }) => {
  ...
  // Go to Vault page
  await page.locator('li').filter({ hasText: 'VaultThis service allows you' }).getByLabel('Subscribe service').click();
  await page.getByRole('button', { name: 'Continue' }).click();
  await page.getByRole('link', { name: 'Vault' }).click();
  await expect(page.getByRole('heading', { name: 'Partner Vault' })).toBeVisible();


  // Upload file
  await page.getByLabel('Add new file').click();
  await page.getByPlaceholder('This is a short paragraph to describe the file.').fill('description');
  ...
  // Filter ok
  await page.getByPlaceholder('Search with filename...').click();
  await page.getByPlaceholder('Search with filename...').fill('a');

  await expect(page.getByRole('cell', { name: 'teste2e.pdf' })).not.toBeVisible();
});
