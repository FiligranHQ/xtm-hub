...
export default async (reportData) => {
  ...
  const url = process.env.TEAMS_WEBHOOK_E2E;
  const failedImage = [];
  const { name, dateH, durationH, summary, htmlPath } = reportData;
  ...
  const title = `- ${process.env.GITHUB_PR_TITLE} - ${name} ${dateH} (${durationH})`;
  ...
  if (summary.passed.value === summary.tests.value) {
    ...
    directories.forEach((directory) => {
      if (directory.includes('attachments')) {
        return;
      }
      description += `\\\n - ${directory}`;
  ...
  try {
    ...
    const responseBody = await response.json(); // Optional: Handle the response if necessary
    console.log('Message successfully posted to Teams channel');
    return responseBody; // Return the response if needed
  } catch (err) {
