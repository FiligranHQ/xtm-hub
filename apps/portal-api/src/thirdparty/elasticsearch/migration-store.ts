...
import { ElasticSearchService } from './client.js';

const defaultEsClient = new ElasticSearchService();

const MIGRATION_INDEX = 'migrations';
const MIGRATION_LOCK_INDEX = 'migrations_lock';
...
const LOCK_DOC_ID = 'current-lock';
const lockDoc = {
  type: 'migration-lock',
  command: 'up',
...
export function createEsStateStorage(
  ...
  const esClient = client || defaultEsClient;
