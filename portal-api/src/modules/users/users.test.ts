...
import { PLATFORM_ORGANIZATION_UUID, ROLE_USER } from '../../portal.const';
...
describe('User helpers - createNewUserFromInvitation', async () => {
  it('should be add new user with Role user only in an existing Organization', async () => {
    const testMail = `testCreateNewUserFromInvitation${uuidv4()}@internal.fr`;
    await createNewUserFromInvitation(
      testMail,
      PLATFORM_ORGANIZATION_UUID as OrganizationId
    );
